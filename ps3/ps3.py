...
def get_word_score(word, n):
    ...
    score_1=0
    for w in word:
        score_1 += SCRABBLE_LETTER_VALUES[w]
...
def is_valid_word(word, hand, word_list):
    ...
    if "*" in word:
        for vowel in VOWELS:
            all_words.append(word.replace("*",vowel))
    else: all_words.append(word)

    match = any(w in word_list for w in all_words)
    
    if match:
        for letter in word:
                if check_hand.get(letter,-1) > 0:
                   check_hand[letter] -= 1
                else: 
                   return False
    else: return False
...
def substitute_hand(hand, letter):
    ...
    if letter in hand:
        ...
        # get list of all letters, subtract letters already in hand
        choices = VOWELS+CONSONANTS
        for k in hand.keys():
            choices = choices.replace(k,"")
        #generate a random letter 
        r = random.choice(choices)
        # add new letter to the hand
        hand[r]=hand[letter]
        #remove old letter from the hand
        del hand[letter]
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    subs = 1
    replay = 1
    # ask user to enter total number of hands
    num_hand = int(input("Enter total number of hands: "))
    # while there are hands left
    while num_hand > 0:
        # create new hand and display it   
        hand = deal_hand(HAND_SIZE)
        display_hand(hand)
        # if substituations remain
        if subs>0:
            #ask if they want to make a substitution
        ...
        if replay >0:
            ...
            if input("Would you like to replay the hand? ") == 'yes':
                ...
                hand_score = max(new_score, hand_score)
                replay -=1
        #update total score
        tot_score += hand_score
        #update number of hands
        num_hand -= 1
    #returen total score
    print("Total score for all hands: ", tot_score)
    return tot_score
